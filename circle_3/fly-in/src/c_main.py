...
def _print_capacity_info(simulator):
    zone_usage = {
            zone_name : 0
    ...
    for drone in simulator.drones:
        if not drone.current_zone is None:
    ...
    for connection in simulator.graph.connections.values():
        print(
            ...
            f"{len(connection.occupants)}/{connection.max_link_capacity})"
        )
